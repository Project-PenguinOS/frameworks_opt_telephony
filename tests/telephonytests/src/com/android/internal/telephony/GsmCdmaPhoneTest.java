...
// QTI_END: 2023-11-09: Telephony: Remove legacy subscription code
    ...
    @Test
    @SmallTest
    public void testDeviceIdAllZeros() throws Exception {
        replaceInstance(GsmCdmaPhone.class, "mImei", mPhoneUT, null);
        String[] respId = new String[]{"000000000000000", "00", "esn", "meid"};
        Message msg = mPhoneUT.obtainMessage(Phone.EVENT_GET_DEVICE_IDENTITY_DONE,
                new AsyncResult(null, respId, null));
        mPhoneUT.sendMessage(msg);
        processAllMessages();

        assertNull(mPhoneUT.getDeviceId());
        assertNull(mPhoneUT.getImei());
        assertEquals(Phone.IMEI_TYPE_UNKNOWN, mPhoneUT.getImeiType());
    }

    @Test
    @SmallTest
    public void testImeiInfoAllZeros() {
        ImeiInfo imeiInfo = new ImeiInfo();
        imeiInfo.imei = "000000000000000";
        imeiInfo.svn = "00";
        imeiInfo.type = ImeiInfo.ImeiType.PRIMARY;

        Message msg = mPhoneUT.obtainMessage(Phone.EVENT_GET_DEVICE_IMEI_DONE,
    ...
    @Test
    @SmallTest
    public void testImeiValid() throws Exception {
        replaceInstance(GsmCdmaPhone.class, "mImei", mPhoneUT, null);
        String[] respId = new String[]{"123456789012345", "01", "esn", "meid"};
        Message msg = mPhoneUT.obtainMessage(Phone.EVENT_GET_DEVICE_IDENTITY_DONE,
                new AsyncResult(null, respId, null));
        mPhoneUT.sendMessage(msg);
        processAllMessages();

        assertEquals("123456789012345", mPhoneUT.getDeviceId());
        assertEquals("123456789012345", mPhoneUT.getImei());
    }
}
